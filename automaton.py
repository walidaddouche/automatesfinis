    ...
    """
        ...
        def forward(re: str, ex: str):
            if re[self.hd] == ex:
                ...
                re_error("forward", ex, re[self.hd])

        def e(re: str) -> Optional[str]:
            if re[self.hd] == "(" or elem(re):
                return ebis(re, c(re))
            else:
                re_error("e", "( or symbol", re[self.hd]); return None

        def ebis(re: str, h: str) -> Optional[str]:
            if re[self.hd] in "+":
                forward(re, '+'); return h + ebis(re, c(re)) + "+"
            elif re[self.hd] in ")$":
                return h
            else:
                re_error("ebis", ")+$ or symbol", re[self.hd]); return None

        def c(re: str) -> Optional[str]:
            if re[self.hd] == "(" or elem(re):
                return cbis(re, k(re))
            else:
                re_error("c", "( or symbol", re[self.hd]); return None

        def cbis(re: str, h: str) -> Optional[str]:
            if re[self.hd] == "(" or elem(re):
                return h + cbis(re, k(re)) + "."
            elif re[self.hd] in "+)$":
                return h
            else:
                re_error("cbis", "()+$ or symbol", re[self.hd]); return None

        def k(re: str) -> Optional[str]:
            if elem(re):
                r = re[self.hd]; forward(re, r)
            elif re[self.hd] == '(':
                forward(re, '('); r = e(re); forward(re, ')')
            else:
                re_error("k", "( or symbol", re[self.hd]); return None
            return kbis(re, r)
        ...
        def kbis(re: str, h: str) -> Optional[str]:
            ...
            if re[self.hd] == '*':
                forward(re, '*'); return h + "*"
            elif re[self.hd] in "+()$" or elem(re):
                return h
            else:
                re_error("kbis", "()+$* or symbol", re[self.hd]); return None
        ...
        if self.hd == len(self.exp):
            ...
            return error("Stopped at index {} \"{}\"".format(self.hd, self.exp[self.hd]))
